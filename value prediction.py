...
y_scaled = scaler.fit_transform(y.values.reshape(-1, 1))  # Scaling y
...
y_gbm_pred_original = scaler.inverse_transform(y_gbm_pred.reshape(-1, 1)).ravel()

# Print the results for original scale
print(f"Original Scale R² Score: {r2_score(y_test_original, y_gbm_pred_original):.4f}")
print(f"Original Scale Mean Absolute Error: {mean_absolute_error(y_test_original, y_gbm_pred_original):.4f}")
print(f"Original Scale Mean Squared Error: {mean_squared_error(y_test_original, y_gbm_pred_original):.4f}")

# Make predictions for the entire dataset
y_gbm_pred_full = gbm_model.predict(X)

# Ensure the same scaler used for scaling y is used for inverse transformation
y_gbm_pred_full_original = scaler.inverse_transform(y_gbm_pred_full.reshape(-1, 1)).ravel()

# Clip negative predictions to 0
#y_gbm_pred_full_original = np.clip(y_gbm_pred_full_original, 0, None)

# Add predictions to the dataset
decoded_X_full = X.copy()
decoded_X_full['Predicted_Crop_Yield_Mt_Per_Ha'] = y_gbm_pred_full_original

# Decode categorical columns back to their original labels
...
decoded_X_full.to_csv('predicted_crop_yields_full_new.csv', index=False)
print("Predicted crop yields saved to 'predicted_crop_yields_full_new.csv'")
